...
function cleanupPreviousContent() {
  ...
  const carousel = document.querySelector("#carousel-overlay-generic .carousel-inner");
  if (carousel) {
    carousel.innerHTML = "";
  }

  // Clear feature highlights
  const muItems = document.getElementById("muItems");
  if (muItems) {
    muItems.innerHTML = "";
  }
...
// Modify the existing AJAX call to include cleanup
...
// Clean up previous content before making the new request
...
$.ajax({
  ...
  .done(function (data) {
    ...
    if (data.B50Desc != null) {
      unitDescripionTemplate += `
		<div id="scrollDescription" class="container-fluid d-print-none" style="padding: 10px 0; border-top: solid 0px #ededed; border-bottom: solid 1px #ededed; background: #fff;">
    ...
    if (data.AccessoryItems.length) {
      featuresHeader += `
		<div id="scrollFeatures" class="container-fluid d-print-none" style="padding: 10px 0; border-bottom: solid 1px #ededed; background: #fff;">
		<h3 class="bold text-center">
    ...
    var featuresTemplate = `
		${featuresHeader}
		<div class="container-fluid collapse d-print-none" id="collapseFeatures">
			<div class="row" style="max-width: 1700px; margin: 0 auto;" id="muItems"></div>
		</div>
	`;
  ...
  .fail(function (jqXHR, textStatus, errorThrown) {
    ...
    const errorMessage = `<div class="alert alert-danger">Failed to load data for stock number ${stockNum}</div>`;
